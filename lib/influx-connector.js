...
exports.initialize = function initializeDataSource(dataSource, callback) {
  ...
  dataSource.connector = new InfluxDBConnector( settings, dataSource['modelBuilder']['models'] );

  if (dataSource.connector.settings.buffer) {
    var j = schedule.scheduleJob({ rule: '*/1 * * * * *' }, function(){
      var now = new Date().getTime();
      if (now >= dataSource.connector.startTime && now < dataSource.connector.endTime && dataSource.connector.buffer.length > 0) {
...
InfluxDBConnector.prototype.getMinimumDuration = function(models) {
  ...
  durations.forEach(function(duration) {
    ...
    if (!(minDuration) || (parsedDuration < parse(minDuration) && duration !== "0s")) {
...
InfluxDBConnector.prototype.bufferData = function(model, data, cb) {
  ...
  if ((this.settings.buffer) && (new Date().getTime() < this.lastWrite + this.settings.bufferTime) && (this.buffer.length < this.settings.bufferTime)) {
    cb(null, data);
  } else {
...
InfluxDBConnector.prototype.clearBuffer = function(cb) {
  var connector = this;
  this.client.writePoints(this.buffer, {database: this.settings.database}).then(function() {
    connector.buffer = [];
    ...
    if (filter.select) {
      if (Array.isArray(filter.select)) {
        filter.select.forEach(function(selector, index) {
          var [err, q, aggregator, selector, transformer] = self.buildSelect(selector);
          if (err) {
            cb(err)
            return;
          } else {
        ...
        var [err, q, aggregator, selector, transformer] = self.buildSelect(filter.select);
        if (err) {
          cb(err)
          return;
        } else {
      ...
      var [err, q, aggregator, selector, transformer] = self.buildSelect(selectObj);
      if (err) {
        cb(err)
        return;
      } else {
        ...
        if (err) {
          ...
          if (filter.group) {
            if (filter.group.indexOf('time') !== -1) {
              if ((aggregator !== null || selector !== null || transformer !== null) && filter.where &&
              ...
              !(transformer === 'nonnegativedifference' && aggregator === null)) {
                ...
                cb(new Error('This query does not allow the response to be grouped by time'))
              }
            } else {
              query = query + ' GROUP BY ' + filter.group;
            }
...
InfluxDBConnector.prototype.buildSelect = function(obj) {
  ...
  if (obj.transformer) {
    ...
    if (transformerMap.hasOwnProperty(transformer)) {
      ...
      return [new Error(transformer + ' is currently not supported as a transformer'), null, null, null, null];
    }
  }
  ...
  if (obj.selector) {
    if (transformer === null) {
      ...
      if (selectorMap.hasOwnProperty(selector)) {
        ...
        return [new Error(selector + ' is currently not supported as a selector'), null, null, null, null];
      }
    } else {
      // rpCb(new Error('A selector cannot be combined with a transformer'), null);
      return [new Error('A selector cannot be combined with a transformer'), null, null, null, null];
    }
  }
  ...
  if (obj.aggregator) {
    if (selector === null) {
      ...
      if (aggregatorMap.hasOwnProperty(aggregator)) {
        ...
        return [new Error(aggregator + ' is currently not supported as an aggregator'), null, null, null, null];
      }
    } else {
      // rpCb(new Error('An aggregator cannot be combined with a selector'), null);
      return [new Error('An aggregator cannot be combined with a selector'), null, null, null, null];
    }
  } else {
    if (transformer === 'movingaverage') {
      // rpCb(new Error(transformer + ' needs to be combined with an aggregator'), null);
      return [new Error(transformer + ' needs to be combined with an aggregator'), null, null, null, null];
    }
  }
  ...
  if (obj.property) {
    ...
    if (aggregator === 'countdistinct') {
      ...
      return [new Error(aggregator + ' requires the specification of a property'), null, null, null, null];
    }
    if (selector === 'bottom' || selector === 'top') {
      return[new Error(selector + ' requires the specification of a property'), null, null, null, null];
      // return;
    }
  }
  ...
  if (obj.selectorParameter) {
    if (selector === 'bottom' || selector === 'percentile' || selector === 'sample' || selector === 'top') {
      subQuery = subQuery + ',' + obj.selectorParameter;
    }
  } else {
    if (selector === 'bottom' || selector === 'percentile' || selector === 'sample' || selector === 'top') {
      return[new Error(selector + ' requires the specification of a parameter'), null];
      // return;
    }
  }
  if (selector !== null) {
  ...
  if (obj.transformerParameter) {
    if (transformer === 'derivative' || transformer === 'elapsed' || transformer === 'movingaverage' || transformer === 'nonnegativederivative') {
      subQuery = subQuery + ',' + obj.transformerParameter;
    }
  } else {
    if (transformer === 'movingaverage') {
      return[new Error(transformer + ' requires the specification of a parameter'), null, null, null, null];
      // return;
    }
  }
  if (transformer !== null) {
...
InfluxDBConnector.prototype.getTimeLimits = function(filter) {
  ...
  if (filter && filter.where) {
    if (filter.where.and) {
      var timeFilters = filter.where.and.filter(f => {return f.time !== undefined && f.time !== null});
...
InfluxDBConnector.prototype.getConsideredRPs = function(filter) {
  ...
  self.sortDurations(Object.keys(self.retentionPolicies)).forEach(function(duration) {
...
InfluxDBConnector.prototype.all = function all(model, filter, callback) {
  ...
  var readings = [];
  async.eachSeries(consideredRPs, function(rp, rpCb) {
      ...
      for (var i = readings.length - 1; i >= 0; i--) {
        ...
        if (response.length == 0) {
          ...
          for (var j = 0; j < rpReadings.length; j++) {
            ...
            if (readings[i+1].length > 1) {
              if (new Date(reading.time).getTime() > (new Date(lastReading.time).getTime() + (new Date(lastReading.time).getTime() - new Date(secondToLastReading.time).getTime()))) {
...
//  */
InfluxDBConnector.prototype.updatePoint = function updatePoint(model, data, point) {
